...
| Categoria | Estado |
| --- | --- |
| Implementado | CRUD de consultorias, productor RabbitMQ, JWT, Actuator |
...
| No evidenciado | consumidor oficial de `Consultoria_Solicitada` fuera de la topologia documentada |

## 1. Descripcion general

Microservicio encargado de registrar y consultar solicitudes de consultoria. Expone operaciones REST del dominio y publica un evento RabbitMQ cuando una solicitud se registra correctamente.

## 2. Rol dentro de la arquitectura

- API Gateway: entrada oficial para `/api/v1/consultorias/**`.
...
`Frontend -> API Gateway -> Consultorias -> MySQL`

Flujo asincrono:

`Consultorias -> RabbitMQ -> consumidor downstream`
...
| --- | --- |
| Puerto interno | `8086` |
| Configuracion | `${SERVER_PORT:8086}` |
...
| `DB_NAME` | opcional, default `innovatech_consultorias` | requerida |
| `DB_USERNAME` | opcional | requerida |
| `DB_PASSWORD` | opcional | requerida |

Compatibilidad adicional:

- `CONSULTORIAS_MYSQL_HOST`
- `CONSULTORIAS_MYSQL_PORT`
- `CONSULTORIAS_MYSQL_DATABASE`
- `CONSULTORIAS_MYSQL_USERNAME`
- `CONSULTORIAS_MYSQL_PASSWORD`
...
| Metodo | Ruta | Uso |
| --- | --- | --- |
| `POST` | `/api/v1/consultorias` | crear solicitud |
| `GET` | `/api/v1/consultorias` | listar solicitudes |
| `GET` | `/api/v1/consultorias/{id}` | obtener solicitud |
| `PUT/PATCH` | `/api/v1/consultorias/**` | actualizaciones del dominio |
...
docker build -t innovatech-consultorias .
```
...
- `Pendiente runtime`: publicacion real del evento `Consultoria_Solicitada` y validacion completa en AWS.
